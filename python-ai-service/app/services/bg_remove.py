...
def remove_background(image_bytes: bytes, background_colour: str = "white") -> bytes:
...
def _resolve_colour(colour: str) -> tuple:
    ...
    raise ValueError(
        ...
        f"Use one of {list(SUPPORTED_COLOURS.keys())} or a hex string like '#ffffff'."
    )
...
def _hex_to_rgba(hex_colour: str) -> tuple:
    ...
    h = hex_colour.lstrip("#")
    # some hex code has length of three below written code is converting into a length of 6
    if len(h) == 3:
